...
\title{A Comparative Analysis of the Blossom Algorithm for Surface Codes} % Article title, use manual lines breaks (\\) to beautify the layout
...
\end{figure}
    \item \textbf{Iterative Process}: Finally, repeat the search for augmenting paths, contraction, and expansion until no more augmenting paths can be found. The whole process and code are presented in Figure 5.
\begin{figure} % Single column figure
    \setlength{\abovecaptionskip}{0.cm} %调整标题上方的距离   
    \setlength{\abovecaptionskip}{0.cm} 
